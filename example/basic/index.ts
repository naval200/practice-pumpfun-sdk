...
const main = async () => {
  ...
  if (!process.env.DEVNET_RPC_URL) {
    console.error("Please set DEVNET_RPC_URL in .env file");
    console.error(
      "Example: DEVNET_RPC_URL=https://api.devnet.solana.com"
    );
    return;
  }

  let connection = new Connection(process.env.DEVNET_RPC_URL || "");
